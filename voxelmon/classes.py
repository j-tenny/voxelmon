...
class CanopyBulkDensityModel:
    ...
    def fit(self, profileData, lidarValueCol, biomassCols, classIdCol='class', plotIdCol='Plot_ID', heightCol='height', sigma=1.2, minHeight=1, fitIntercept=False,twoStageFit=False, alpha_test = np.logspace(-3,0,19)):
        ...
        # Apply gaussian smoothing filter to profile data for each plot
        ...
        for i in range(alpha_test.shape[0]):
            ...
            coef = 1 / coef
            coef[coef <= 0] = 0
            mass_ratio = pd.Series(coef,index=biomassCols)
            self.mass_ratio = mass_ratio.to_dict()
...
class _CenteredRidge(Ridge):
    def __init__(self, alpha=1.0, target=1.0, **kwargs):
        super().__init__(alpha=alpha, **kwargs)
        self.target = target

    def fit(self, X, y):
        # Adjust y to include the target value in the penalty
        X_adj = X
